...
def print_log(message: str):
    ...
    logging.info(f"[TXT] {message}")

def is_separator(line: str) -> bool:
    """Checks if a string starts with a text separator like '===', '---', or '~~~'"""
    return bool(re.match(r"^[-=~_*]{3,}", line.strip()))

def process_single_txt(file_path: Path, output_dir: Path) -> int:
    """
    Worker function to process a single .txt file.
    
    Side-effects:
        - Reads from `file_path`.
        - Writes to `{output_dir}/{file_path.stem}_txt_chunks.json` using `write_chunks_to_json`.
        
    Args:
        file_path (Path): Path to the source .txt document.
        output_dir (Path): Path to the destination directory for chunks.
        
    Returns:
        int: Total number of chunks written.
    """
    with file_path.open("r", encoding="utf-8", errors="ignore", newline="\n") as t:
        text = t.read()
    ...
    for para in paragraphs:
        lines = para.split("\n")
        has_sep = any(is_separator(l) for l in lines)
        if has_sep and current_unit:
    ...
    output_path = output_dir / f"{file_path.stem}_txt_chunks.json"
...
def main(file_paths, async_mode):
    ...
    print_log(f"=== Starting TXT Chunking Run: {get_timestamp()} ===")
    ...
    if async_mode:
        total_chunks = asyncio.run(process_files_async(process_single_txt, paths, chunks_dir))
    else:
        total_chunks = process_files_sync(process_single_txt, paths, chunks_dir)
